...
def full_run(dataset_name="MNIST", nb_teachers=200):
    
    
    params = {"threshold": 150, "sigma_threshold": 120, "sigma_gnmax": 40, "epsilon": 3, "delta" : 10e-5}
    ...
    #student.util_train_student(dataset_name=dataset_name, n_epochs=100)
    ...
    #noise_vote_array = pate_data.query_teachers("noise_MNIST", nb_teachers)
    ...
    noise_votes = pate_main.inference_pate(vote_array=noise_vote_array, threshold=params["threshold"], sigma_threshold=params["sigma_threshold"], sigma_gnmax=params["sigma_gnmax"], epsilon=params["epsilon"], delta=params["epsilon"], num_classes=10, savepath=noise_label_path) 
    
    #then train the student on Gaussian noise    
    student.util_train_student(dataset_name="noise_MNIST", n_epochs=60, lr=0.001, optimizer="Adam", kwargs=params)
    

#full_run(nb_teachers=200)
